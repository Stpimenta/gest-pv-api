...
namespace c___Api_Example.Repository
{
    public class CaixaRepositorio : ICaixaRepositorio
    {
        ...
        private async Task<CaixaModel> GetCaixaById(int id)
        {
            CaixaModel? caixa = await _ibpvDataBaseContext.Caixa.FindAsync(id);
        ...
        public async Task<int> UpdateCaixa(int id, CaixaPutDto caixaDto)
        {
            CaixaModel? caixa = await GetCaixaById(id);
            
            if (caixa is null)
                throw new Exception("caixa não encontrado");
            
           caixa.Nome =  caixaDto.Nome;
