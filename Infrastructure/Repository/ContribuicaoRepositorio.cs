...
namespace c___Api_Example.Repository
{
    public class ContribuicaoRepositorio : IContribuicaoRepositorio
    {
        ...
        public async Task<ContribuicaoModel> GetContribuicaoById(int id)
        {
            var contribuicao = await _ibpvDataBaseContext.Contribuicao
                .Include(c => c.Images)
        ...
        public async Task<int> UpdateContribuicao(int id, ContribuicaoModel model)
        {
            var dbContribuicao = await _ibpvDataBaseContext.Contribuicao
                .Include(c => c.Images)
                .FirstOrDefaultAsync(c => c.Id == id);

            if (dbContribuicao is null)
                throw new Exception("Contribuição não encontrada");

         
            var caixa = await _ibpvDataBaseContext.Caixa.FindAsync(dbContribuicao.IdCaixa);
            if (caixa is null)
                throw new Exception("Caixa não encontrado");
            
            caixa.ValorTotal -= dbContribuicao.Valor;

            if (model.IdCaixa != dbContribuicao.IdCaixa)
            {
                var newCaixa = await _ibpvDataBaseContext.Caixa.FindAsync(model.IdCaixa);
                newCaixa.ValorTotal += model.Valor;
                dbContribuicao.IdCaixa = model.IdCaixa;
            }
            else
            {
                caixa.ValorTotal += model.Valor;
            }
            ...
            dbContribuicao.Images.Clear();

            foreach (var img in model.Images)
            {
                dbContribuicao.Images.Add(img);
            }

            await _ibpvDataBaseContext.SaveChangesAsync();
            return dbContribuicao.Id;
        }
